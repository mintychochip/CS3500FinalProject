...
def extract_datetime_components(df: pd.DataFrame) -> pd.DataFrame:
  df['DateRptdMonth'] = df['Date Rptd'].dt.month
  df['DateRptdYear'] = df['Date Rptd'].dt.year
  df['DateRptdDay'] = df['Date Rptd'].dt.day
  df['DateOccMonth'] = df['DATE OCC'].dt.month
  df['DateOccYear'] = df['DATE OCC'].dt.year
...
def generate_features(df: pd.DataFrame) -> pd.DataFrame:
  ...
  df['HourOfDay'] = df['TIME OCC'].dt.hour

  # Holiday feature: Check if a date is a holiday
  df['IsHoliday'] = df['DATE OCC'].apply(
      lambda x: x.date() in us_holidays)
  # Replacing the 'AvgCrimeInArea' by counting the number of crimes in each area
  df['CrimeCountInArea'] = df.groupby('AREA')['Crm Cd'].transform('count')

  # Replacing 'VictimAgeToCrimeRate' with a proxy based on crime rates per age bucket (without using 'Target')
  df['VictimAgeToCrimeRate'] = df.groupby('AgeBucket')['Crm Cd'].transform(
    'count') / df.groupby('AgeBucket')['Vict Age'].transform('count')
...
def clean_data(df: pd.DataFrame, file_path: str) -> pd.DataFrame:
  ...
  df = apply_one_hot_encoding(df,
                              ['AREA', 'Part 1-2', 'Vict Descent', 'Vict Sex',
                               'TimeOfDay', 'Season', 'AgeBucket', 'HourOfDay',
                               'MonthName', 'DayOfWeek', 'IsHoliday'])
  df['Mocodes'] = df['Mocodes'].fillna('').astype(str).str.split()
  df = apply_feature_hashing(df, ['Mocodes'],
                             FeatureHasher(n_features=32, input_type='string'))
