...
def get_season_label(dt: datetime) -> Optional[str]:
    ...
    for (start, end), season in season_map.items():
        if start <= dt.month <= end or (start > end and (dt.month >= start or dt.month <= end)):
            return season
...
def apply_one_hot_encoding(df: pd.DataFrame, features: List[str]) -> pd.DataFrame:
...
def remove_unwanted_columns(df: pd.DataFrame) -> pd.DataFrame:
    cols_to_drop = ['AREA NAME', 'Crm Cd Desc', 'Premis Desc', 'Weapon Desc', 'Status Desc']
